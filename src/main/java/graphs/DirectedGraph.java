...
public class DirectedGraph<V extends Identifiable, E> {
     ...
     * 1. the vertices map stores all vertices by their identifying id (which
     * prevents duplicates)
     * 2. the edges map stores all directed outgoing edges by their from-vertex and
     * then in the nested map by their to-vertex
     * 3. there can only be two directed edges between any two given vertices v1 and
     * v2:
     * one from v1 to v2 in edges.get(v1).get(v2)
     * one from v2 to v1 in edges.get(v2).get(v1)
     * 4. every vertex instance in the key-sets of edges shall also occur in the
     * vertices map and visa versa
     **/
    ...
    public Collection<V> getNeighbours(V fromVertex) {
        ...
        // TODO retrieve the collection of neighbour vertices of fromVertex out of the
     ...
     * (only the out-going edges directed from 'fromVertex' towards a neighbour
     * shall be included
     *
     * @param fromVertex
     * @return null if fromVertex cannot be found in the graph
    ...
    /**
     * Adds newVertex to the graph, if not yet present and in a way that maintains
     ...
     *
     * @param newVertex
     * @return the duplicate of newVertex with the same id that already exists in
     *         the graph,
     ...
     * No change shall be made if a directed edge already exists between these
     * vertices
     *
     ...
     *         returns null if no connection has been set up between these vertices
     *         in the specified direction
     */
    public E getEdge(V fromVertex, V toVertex) {
        if (fromVertex == null || toVertex == null)
            return null;
        Map<V, E> edge = edges.get(fromVertex);
        return edge.get(toVertex);
        // TODO retrieve the directed edge between vertices fromVertex and toVertex from
        // the graph
    ...
    /**
     * calculates and returns the total number of directed edges in the graph data
     * structure
     *
    ...
    public int getNumEdges() {
        ...
        int totalEdges = 0;
        for (Map<V, E> innerMap : edges.values()) {
    ...
    public class DGPath {
        ...
        /**
         * representation invariants:
         * 1. vertices contains a sequence of vertices that are connected in the graph
         * by a directed edge,
         * i.e. FOR ALL i: 0 < i < vertices.length:
         * this.getEdge(vertices[i-1],vertices[i]) will provide edge information of the
         * connection
         * 2. a path with one vertex has no edges
         * 3. a path without vertices is empty
         * totalWeight is a helper attribute to capture additional info from searches,
         * not a fundamental property of a path
         * visited is a helper set to be able to track visited vertices in searches, not
         * a fundamental property of a path
         **/
    ...
    /**
     * Uses a depth-first search algorithm to find a path from the start vertex to
     ...
     *         returns null if either start or target cannot be matched with a
     *         vertex in the graph
     *         or no path can be found from start to target
     */

    private boolean dfs(V current, V end, DGPath path) {
        ...
        for (V neighbor : edges.get(current).keySet()) {
            if (!path.visited.contains(neighbor)) {
                boolean found = dfs(neighbor, end, path);
                if (found) {
                    return true;
                }
            }
    ...
    /**
     * Uses a breadth-first search algorithm to find a path from the start vertex to
     * the target vertex in the graph
     * All vertices that are being visited by the search should also be registered
     * in path.visited
     *
        ...
        while (!queue.isEmpty()) {
            ...
            if (node.equals(target)) {
                while (!node.equals(start)) {
                    path.vertices.add(node);
                    node = parent.get(node);
                }
                path.vertices.add(start);
                Collections.reverse((List) path.vertices);
                return path;
            }

            for (V neighbor : edges.get(node).keySet()) {
                if (!path.visited.contains(neighbor)) {
    ...
    private class DSPNode implements Comparable<DSPNode> {
        ...
        protected double weightSumTo = Double.MAX_VALUE; // sum of weights of current shortest path to this node's
                                                         // vertex
     ...
     * @param weightMapper provides a function, by which the weight of an edge can
    ...
    public DGPath dijkstraShortestPath(String startId, String targetId,
        ...
        // you may choose a different approach of tracking progress of the algorithm, if
        // you wish
